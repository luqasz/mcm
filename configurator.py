...
class configurator:
	...
	def __init__( self, host, user, pw ):
		...
		self.run_time = None
		rosapi.parent_logger = 'mcm.configurator'
...
class comparison:
	...
	def default( self, wanted, present ):
		...
		self.log.debug( 'dynamic ids = {0}'.format( dyn_ids ) )
		...
		for wanted_rule in wanted:
			...
			if not present_rule:
				...
				if '.id' not in present_rule and present_rule and len( wanted ) > 1:
					self.log.critical( 'more than one rule specified in no \'.id\' menu level. skipping...' )
					return ( [], [], [] )
	...
	def uniqKeyNoOrder( self, wanted, present, key, offset = 0, mng_def = False ):
		...
		all_ids = [dict['.id'] for dict in present if dict.get( '.id' )]
